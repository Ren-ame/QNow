...
| # | 내용 | 비고 |
|---|------|------|
| 1 | 신규 등록된 장소를 지도 마커에 표시 | 커스텀 장소 핀을 일반 마커와 구분되는 디자인으로 표시 |
| 2 | 커스텀 장소 수정 기능 | 등록자 본인이 장소 정보 수정 가능, 비어드민 유저는 어드민에게 삭제 요청 가능 |
| 3 | 커스텀 장소 대기정보 표시 | 카드 클릭 → 대기정보 입력은 동작하나, `fetchCustomPlaces`가 `waitTime: 0` 하드코딩이라 입력값이 화면에 반영 안 됨 → `wait_times` 테이블에서 커스텀 장소 ID 기준으로 조회해서 반영 필요 |
