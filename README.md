...
- **검색창 z-index 수정** — 검색 추천 드롭다운이 반경 패널·필터 패널 뒤로 숨는 문제 해결. 검색바 wrapper를 `z-20` → `z-50`으로 상향해 모든 UI 위에 표시되도록 수정.
- **검색 반경 독립화** — 검색바에서 텍스트 입력 시 거리 슬라이더(반경 필터)에 영향받지 않도록 카카오 최대 반경(20km) 고정. "이 지역 재검색"·필터 변경·위치 이동은 기존 `searchRadius` 유지.
- **검색 추천 선택 시 카드 상단 고정 복원** — `handleSuggestionSelect`에서 `setSelectedPlace` 대신 `handleMarkerClick`을 호출하도록 수정해 선택된 장소가 주변 대기 정보 목록 최상단으로 이동하는 동작 복원.
- **바텀 시트 개선** — 스냅 포인트를 3단계(10% 숨김 / 45% 기본 / 85% 확장)로 재설계. 핸들만 보이는 상태까지 완전히 내릴 수 있음. `liveHeight` ref 도입으로 드래그 종료 시 stale closure 버그 수정.
- **플로팅 버튼 동적 위치** — 현재 위치·이 지역 재검색·신규 등록 세 버튼이 바텀 시트 높이에 따라 실시간으로 함께 이동. 드래그 중에는 `transition: none`으로 즉각 따라가고, 스냅 시 `300ms ease-out` 애니메이션 적용.
...
- **카카오 계정 전환 기능 추가** — 앱 로그아웃 후 다른 카카오 계정으로 전환 불가하던 문제 해결.
  ...
  - `switchKakaoAccount` 함수 추가 — `prompt=select_account`로 계정 선택 화면 강제 노출
  - 메뉴 로그인 버튼 아래 "다른 계정으로 로그인" 링크 추가 (이 버튼에서만 기억하기 무시)
...
| # | 내용 | 비고 | 상태 |
|---|------|------|------|
| 1 | 첫 로그인 후 재로그인 시 이전 세션이 남아 다른 계정으로 전환 불가 | 카카오 SSO 세션이 원인. "다른 계정으로 로그인"(prompt=select_account) 추가로 해결 | 완료 |
